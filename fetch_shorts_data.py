...
# API endpoint (using local API on port 8001)
API_BASE_URL = "http://localhost:8001"

async def fetch_video_data(client: httpx.AsyncClient, url: str, index: int) -> Dict[str, Any]:
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"[{index + 1}/{len(urls)}] ✓ Success - {data.get('data', {}).get('snippet', {}).get('title', 'N/A')[:50]}")
            return {
                "url": url,
                "status": "success",
                "data": data
...
async def fetch_all_videos():
    ...
    async with httpx.AsyncClient() as client:
        tasks = [fetch_video_data(client, url, i) for i, url in enumerate(urls)]
...
def main():
    ...
    # Fetch all data
    results = asyncio.run(fetch_all_videos())
    ...
    with open(csv_file, "w", encoding="utf-8") as f:
        ...
        for result in results:
            if result["status"] == "success":
                ...
                f.write(f'"{result["url"]}",N/A,N/A,N/A,0,0,0,{result.get("error", "error")}\n')
